defmodule ElixirTesteTest.EscreveNumeroAleatorioTest do
  ...
  test "2 escritas no arquivo geram numeros diferentes" do
    ...
    primeiro_conteudo = File.read!(Path.join([:code.priv_dir(:elixir_teste), "arquivo.txt"]))

    ElixirTeste.EscreveNumeroAleatorio.escreve()

    segundo_conteudo = File.read!(Path.join([:code.priv_dir(:elixir_teste), "arquivo.txt"]))
    assert primeiro_conteudo != segundo_conteudo
  end

  test "teste com mock" do
